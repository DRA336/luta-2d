...
public class Player2Controller2D : MonoBehaviour
{
    ...
    void Awake(){ motor = GetComponent<CharacterMotor2D>(); actions = GetComponent<FighterActions>(); }
    ...
    void Update(){
        ...
        if (Input.GetKey(defend)) actions.StartDefense(); else actions.StopDefense();
        if (Input.GetKeyDown(jab) && actions.CanAct()) actions.TryJab();
    }
}
